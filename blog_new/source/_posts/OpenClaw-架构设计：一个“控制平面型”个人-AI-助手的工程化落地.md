...
官方推荐路径是 `openclaw onboard`（向导驱动配置、通道、skills、守护进程），CLI 的职责本质是：

- 提供一个稳定的“运维/配置/诊断面”
- 把复杂的安装、升级、迁移、风险提示收敛到 `doctor/onboard/update/logs/...`
...
```text
~/.openclaw/
  openclaw.json                  # 主配置（可能含敏感信息：token/keys/账号等）
  openclaw.json.bak*             # 配置备份（迁移/修复时常见）
  config.json                     # 运行态配置快照/兼容层（视版本而定）
  agents/
    main/
      sessions/                   # 会话转录（JSONL），用于追踪/回放/排障
  workspace/
    AGENTS.md                     # 工作区说明与约定（工作区即契约）
    USER.md / SOUL.md / TOOLS.md  # 人格/用户/工具等引导文件
    memory/                       # 以日期为粒度的记忆/日志文件（Markdown 真源）
    skills -> /Users/zenghao/.agents/skills  # workspace skills（已做软连接）
    docs/ scripts/ temp/           # 文档、脚本、临时产物
  memory/
    main.sqlite                   # 记忆索引库（不是真源；可重建）
  devices/                        # 节点配对信息
  cron/                           # 定时任务定义与运行记录
  browser/
    chrome-extension/             # 浏览器扩展（把现有 tab 挂到 relay 供自动化使用）
```
